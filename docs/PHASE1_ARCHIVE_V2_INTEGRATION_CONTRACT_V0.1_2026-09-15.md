# Phase 1 Archive -> 0700 Minimal V2 Integration Contract V0.1 — 2026-09-15

Status: FROZEN_PRE_IMPLEMENTATION / PRE_OUTCOME / HOLDOUT UNSCORED / ORDER SEND DISABLED

Contract ID: `PHASE1_ARCHIVE_V2_INTEGRATION_V0.1`
...
6. integration envelope preserves source months and all source SHA values;
7. V2 outputs retain integration source/representation/seed-status markers;
8. M5/H4 bars derived through the integration match direct `resample_ohlc` from the same M1 input;
9. no H1/D1 dependency is introduced into current V2;
10. no P&L/Win Rate/broker-fill field is added by integration;
11. existing V2 test suite remains unchanged/pass;
12. archive M1 input is not mutated by V2 integration.
...
`PHASE1_ARCHIVE_V2_INTEGRATION_V0.1 = FROZEN_PRE_IMPLEMENTATION`
